...
def main():
    ...
    parser.add_argument("--micro-batch", type=int, default=None,
                        help="Override per-replica micro-batch; defaults to the stage's micro_batch_size.")
    args = parser.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=True)

    model_cfg = ModelConfig.from_yaml(args.config)
    schedule = schedule_for_config(args.config)

    rank = int(os.environ.get("LOCAL_RANK", 0))
    world_size = int(os.environ.get("WORLD_SIZE", 1))

    stages = [s for s in schedule if args.stage in (s.name, "all")]
    for w in validate_parallel_layout(model_cfg, stages[0].cfg):
        print(f"WARNING: {w}")
    print(f"running stages: {[s.name for s in stages]}")

    for stage in stages:
        print(f"\n=== STAGE: {stage.name} ({stage.fraction_of_compute:.0%} compute) ===")
        micro_batch = args.micro_batch or stage.cfg.micro_batch_size
        data_iter = packed_shard_iter(args.packed_dir, stage.cfg.seq_len, micro_batch)
