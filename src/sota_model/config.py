...
- ~427B dense parameters (verify via ModelConfig().estimate_params_billions()),
  110 layers, d_model 16384
...
class ModelConfig:
    ...
    # training at frontier width. Off by default to preserve the 4.7-class
    # lineage; sota_ultra_5.yaml enables it.
    qk_norm: bool = False
    ...
    def estimate_params_billions(self) -> float:
        ...
        if self.qk_norm:
            norms += self.n_layers * 2 * self.head_dim
...
class TrainingConfig:
    ...
    # Defaults: 8192 × 2 × 16 × 16 = 4,194,304 (the 4.7-class shape).
    global_batch_tokens: int = 4_194_304
    micro_batch_size: int = 2   # sequences per replica per micro-step
    grad_accum: int = 16
    ...
    # tp × pp GPUs. Defaults land 8 × 8 × 16 = 1024 GPUs (the 4.7-class layout).
    dp_degree: int = 16
