...
A 427B dense model does not fit on any single GPU's HBM (~854 GB at bf16; ~427 GB at fp8). One axis isn't enough:
...
```bash
sota-pretrain \
    --config configs/sota_ultra_5.yaml \
...
```bash
sota-pretrain --config configs/sota_ultra_5.yaml --output-dir ./checkpoints --smoke
```
