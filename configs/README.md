...
```
sota_ultra_5.yaml   UltraModel 5-class spec — CURRENT frontier spec (model + training + inference)
sota_4_7.yaml       SuperModel 4.7-class spec — prior generation, retained for lineage
```

`sota_ultra_5.yaml` is the current/default spec. `sota_4_7.yaml` is kept because the
detailed back-trace tables further down derive the 4.7 numbers in full; the same
"7 pinned, the rest operator-committed" methodology applies to `sota_ultra_5.yaml`.

### Current spec: `sota_ultra_5.yaml` (UltraModel 5)

UltraModel 5 supersedes SuperModel 4.7. It is the generally-available configuration of
MathsSchool's new Mythos-class frontier model (same weights as SecretUltraModel 5, plus
production safeguards + fallback to the latest SuperModel). Key deltas vs `sota_4_7.yaml`:

| Knob | 4.7-class | UltraModel 5 |
|---|---|---|
| `d_model` / `n_layers` / `n_q_heads` / `n_kv_heads` / `ffn_dim` | 16384 / 110 / 128 / 16 / 65536 | 18432 / 128 / 144 / 18 / 73728 |
| total params (dense) | ~427B | ~627B (verify: `ModelConfig.from_yaml('configs/sota_ultra_5.yaml').estimate_params_billions()` → 627.06) |
| KV @ 1M, bf16 / fp8 | 880 / 440 GiB | 1152 / 576 GiB |
...
| `capability_targets` | §8 scoreboard @ SuperModel 4.7 | §8 scoreboard @ UltraModel 5 / SecretUltraModel 5 (47 gates) |
| `safety_thresholds` | §4/§5 @ 4.7 | §4/§5 @ UltraModel 5 (14 gates) |
...
```
sota_ultra_5.yaml   (and sota_4_7.yaml)
...
## Back-trace: every field vs `` (worked example: `sota_4_7.yaml`)

The tables below derive the **4.7-class** numbers in full as the worked example. The
same "7 modelcard-pinned, the rest operator-committed" methodology applies unchanged to
`sota_ultra_5.yaml`; see the [Current spec](#current-spec-sota_ultra_5yaml-ultramodel-5)
delta table for the values that move (`d_model`/`n_layers`/`ffn_dim`, params, KV, tokens,
batch, LR, `thinking_budgets.max`, and the re-pinned `capability_targets` /
`safety_thresholds`).
...
Both configs carry additional sections beyond `model:` / `training:` / `inference:` that capture the values needed to **deliver** modelcard targets, even though modelcard doesn't pin them numerically. These are loaded via `sota_model.config.load_implied(path)` — they are NOT consumed by the dataclasses. (The "what it pins" column below shows the 4.7-class values; `sota_ultra_5.yaml` carries the same sections re-pinned to UltraModel 5 — 627B scale, 36T tokens, the §8 UltraModel 5 scoreboard, etc.)
...
shardability, mix percentages, gate numericity, and the special-token/language counts.
CI runs the same gate on every push (`.github/workflows/check.yml`).

## How to override

Don't edit `sota_ultra_5.yaml` for ad-hoc experiments — copy it:
