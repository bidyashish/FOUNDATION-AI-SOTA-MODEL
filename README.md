...
The current spec is **UltraModel 5** ([`configs/sota_ultra_5.yaml`](./configs/sota_ultra_5.yaml)): ~627B dense, 36T training tokens, 95.5% SWE-bench Verified, 94.1% GPQA Diamond, 84.3% Terminal-Bench 2.1, 85.0% OSWorld, 1M-token context (10M agentic with compaction), native multimodal, adaptive thinking with a 131K max thinking budget, and a passing RSP risk profile. The prior-generation **SuperModel 4.7-class** spec ([`configs/sota_4_7.yaml`](./configs/sota_4_7.yaml), ~427B) is retained for lineage, and its numbers are the **worked example** this guide walks through — the methodology is identical, and [`configs/README.md`](./configs/README.md) has the full UltraModel 5 delta table.
...
The table below is the **4.7-class worked-example scoreboard** that the architecture and pipeline walkthrough in the rest of this guide references:
...
A dense decoder-only transformer at the **frontier-dense band** — ~427B active parameters at the 4.7-class worked-example defaults below, in the 400–700B band that 2026 frontier-dense (Llama 4 dense backbone, Claude Opus 4.x, Gemini Ultra dense subset) lives in. The current default `configs/sota_ultra_5.yaml` lands ~627B at the top of the same band (18432 × 128 layers — see the delta table in [`configs/README.md`](./configs/README.md)). All parameters active at inference (no MoE routing). **The full UltraModel 5 architecture reference — block diagram, parameter accounting, QK-norm/z-loss stability kit, sharding map — is [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md); the visual, tensor-shape walkthrough of the forward pass (prefill → decode, KV-cache paging, training step) is [`docs/TRANSFORMER_FLOW.md`](./docs/TRANSFORMER_FLOW.md).**

### 1.1 Topology (worked example: 4.7-class)
...
Total params at these defaults:    ≈ 427 B (run cfg.estimate_params_billions())
                                   UltraModel 5 defaults land ≈ 627 B

```

### 1.1.1 The 2026 frontier-dense design space

`` doesn't pin numbers for `d_model`, `n_layers`, `n_kv_heads`, `ffn_dim`, optimizer, precision, total params, training tokens, or sampling. Only **7** numerical/structural choices are modelcard-pinned (the 1M context, the 2576px / 3.75 MP image cap, the 200K compaction trigger, and adaptive thinking + tier names); the rest are **operator-committed**. The `model:` and `training:` sections of the configs are calibrated against published 2026 frontier-dense practice — `configs/sota_ultra_5.yaml` (UltraModel 5, ~627B) is the current default; `configs/sota_4_7.yaml` (SuperModel 4.7-class, ~427B) is the worked example walked through in this section.
...
| Knob | 2024 typical | 2026 frontier band | 4.7 example → Ultra 5 | Notes |
|---|---|---|---|---|
| `d_model` | 12,288–16,384 | 14,336–20,480 | 16,384 → 18,432 | capability ceiling scales with width |
| `n_layers` | 80–126 | 100–180 | 110 → 128 | depth helps reasoning; cost is wall-clock |
| `n_q_heads` | 64–128 | 96–160 | 128 → 144 | scales with d_model; head_dim=128 stable |
| `n_kv_heads` (GQA) | 8–16 | 8–16 (or MLA) | 16 → 18 | 8:1 group ratio; KV memory bound |
| `head_dim` | 128 | 128 | 128 | stable across the band |
| `ffn_dim` (× d_model) | 3.0–4.0 | 3.5–4.0 | 4.0 | SwiGLU; tapering across depth is modern |
| Total params (B, dense) | 100–500 | 400–700 | 427 → 627 | DeepSeek-V3 is 671B MoE; Llama 4 ~2T MoE → frontier *dense* sits at 400–700B |
| Training tokens (T) | 10–18 | 20–40 | 25 → 36 | ~3× Chinchilla-optimal at 427B (8.5T optimal), ~2.9× at 627B (12.5T optimal); band 20–40T |
...
| Global batch (tok) | 2–8 M | 4–8 M | 4 M → 8 M | stable with ZeRO-3; larger model → larger stable batch |
...
**Total volume:** **20–40 trillion tokens** of pretraining corpus. The current default `configs/sota_ultra_5.yaml` commits to **36T** (627B dense, 8M-token batch); the worked example in this section uses the 4.7-class `configs/sota_4_7.yaml::implied_training_corpus.total_tokens_trillions = 25` at a 4M-token batch, which is roughly **5M to 10M optimizer steps** before any post-training.

This is the 2026 frontier-dense band. Reference points: DeepSeek-V3 trained on 14.8T (Dec 2024); Llama 4 trained on ~30T (early 2025). Chinchilla-optimal at 427B params is ~8.5T (20 tokens/param), so 25T sits at ~3× over optimal — and the frontier band 20–40T spans 2.3–4.7× over optimal. Frontier-dense overtrains because compute is cheaper than the marginal eval gain at the optimal point; pushing past ~5× is uncommon in 2026 public practice.
...
training_config_stage1 = dict(
    ...
    grad_clip=1.0,
    warmup_steps=2_000,
    total_steps=6_000_000,             # 25T tokens / 4M-token batch (5M–10M band, 2.2)
    seq_len=8_192,
...
At the UltraModel 5 shape (128 layers, 18 KV heads, same head_dim) the same arithmetic gives **1152 KiB/token → 1152 GiB at 1M** bf16, halved at the default fp8 KV cache — see `implied_scale` in `configs/sota_ultra_5.yaml`.
...
For a ~427B dense model (worked example), ordered by per-GPU capability (newest first). The 627B UltraModel 5 needs roughly 1.5× per replica — `implied_serving_minimums` in `configs/sota_ultra_5.yaml` pins 8 × B200 fp8 / 6 × B300 / 4 × Rubin R100:
...
Sharding
    Tensor parallel:   8     # within node, NVLink 5 (1.8 TB/s) / NVLink 6 (3.6 TB/s)
    Pipeline parallel: 2     # across nodes, InfiniBand or NVLink Switch fabric
