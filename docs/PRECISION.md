...
| `MXFP8`       |  8+ | 1 | 4 | 3 + **block-shared exponent (32 elem, E8M0)** | bf16-like | bf16-like | training default 2025+ (Blackwell) |
| `MXFP6`       |  6+ | 1 | 3 | 2 + **block-shared exponent (32 elem, E8M0)** | bf16-like | bf16-like | mid-precision serving |
| `MXFP4`       |  4+ | 1 | 2 | 1 + **block-shared exponent (32 elem, E8M0)** | bf16-like | bf16-like | memory-bound serving (Blackwell native) |
| `NVFP4`       |  4+ | 1 | 2 | 1 + **block-shared FP8 scale (16 elem, E4M3)** + per-tensor FP32 | bf16-like | finer than MXFP4 | inference default 2026+ (Rubin native) |
...
**NVFP4** is NVIDIA's own 2026 evolution of MXFP4, introduced with Rubin: smaller block size (16 instead of 32), richer per-block scale (FP8 E4M3 instead of MXFP4's E8M0 power-of-2), and a second-level per-tensor FP32 scale. Empirically NVFP4 retains ~0.5pp more accuracy than MXFP4 at the same bit-width, at the cost of slightly more metadata (16 × 4 + 8 bits per block vs MXFP4's 32 × 4 + 8 — i.e. 4.5 vs 4.25 effective bits/value). Rubin GPUs have dedicated NVFP4 tensor cores; Blackwell emulates NVFP4 in software via its MXFP4 path.
...
NVFP4   ┌──── 16 × fp4_E2M1 ────┐ ┌── E4M3 scale ──┐  + per-tensor FP32 scale
        │ 16 four-bit values    │ │ one byte/block │
        └───────────────────────┘ └────────────────┘
        8 bytes payload + 1 byte E4M3 scale = 9 bytes per 16 values
        ≈ 4.5 effective bits per value (finer scaling than MXFP4)
```
...
### `NVFP4` — NVIDIA's 2026 Rubin-native FP4
Block of **16** FP4 values shares an **FP8 (E4M3) scale**, plus a **per-tensor FP32 master scale**. Two innovations vs MXFP4:

1. **Half the block size (16 vs 32).** Outliers within a block dominate less; effective accuracy is ~0.5pp higher than MXFP4 at the same nominal bit-width on most evals.
2. **FP8 scale instead of E8M0.** The E4M3 scale has 3 bits of mantissa (vs E8M0's zero), so the scale itself is finer-grained — closer to true bf16 dynamic range than MXFP4's power-of-2-only scale.
3. **Two-level scaling.** The per-tensor FP32 scale handles outliers across blocks (inter-block variation) while the per-block FP8 scale handles within-block magnitude.

Effective storage: 16 × 4 + 8 = **4.5 bits per value** (vs MXFP4's 4.25). The 0.25-bit overhead buys the better accuracy; this is the right tradeoff at Rubin's bandwidth-bound regime where memory cost dominates.

NVIDIA Rubin (2026) has **dedicated NVFP4 tensor cores** with hardware-accelerated adaptive compression in Transformer Engine 2. Blackwell can run NVFP4 via software-level conversion to/from its native MXFP4 tensor cores — the format is interoperable, just not hardware-fastpath on Blackwell.

NVFP4 is the **inference default** on Rubin: 50 PFLOPS NVFP4 inference (5× B200's MXFP4 inference throughput), 35 PFLOPS NVFP4 training (3.5×). For frontier-dense models, the practical implication is that the same serving worker shrinks from 4× B200 to 2× Rubin while halving latency.
...
### NVIDIA Rubin (R100 / Vera Rubin NVL144, 2026+)

The successor to Blackwell. Announced January 2026, ramping into production fleets through 2026-2027. Per-GPU specs:

| Property | Value | vs Blackwell |
|---|---|---|
| HBM4 capacity | **288 GB / GPU** | 1.5× (B200 = 192 GB) |
| HBM4 bandwidth | **22 TB/s** | 2.8× |
| NVLink 6 bandwidth | **3.6 TB/s** bidirectional / GPU | 2× |
| NVFP4 inference | **50 PFLOPS** | 5× B200's MXFP4 |
| NVFP4 training | **35 PFLOPS** | 3.5× |
| Transistors | **336 B** | 1.6× |

| Format | Native? | Tensor-core throughput vs Blackwell-bf16 |
|---|:---:|---|
| fp32 (acc) | yes | 1× |
| bf16, fp16 | yes | 1× |
| fp8 (E4M3 / E5M2) | yes | ~2.5× |
| MXFP8 | yes | ~2.5× |
| MXFP6 | yes | ~3.5× |
| MXFP4 | yes | ~4× |
| **NVFP4** | **yes (native)** | **~5× (50 PFLOPS inference)** |
| int8 | yes | ~2.5× |
| int4 | yes | ~4× |

The whole **Vera Rubin platform** is six chips:

| Chip | Role | Key spec |
|---|---|---|
| Rubin GPU | the GPU | 288 GB HBM4, 50 PFLOPS NVFP4 |
| Vera CPU | the host CPU | 88 Olympus cores, 1.5 TB LPDDR5X, 1.2 TB/s |
| Rubin CPX | context-optimized accelerator | optimizes long-context inference (10M+ ctx) |
| ConnectX-9 | NIC | 800 Gb/s per port, 1.6 Tb/s per Rubin GPU |
| NVLink 6 Switch | scale-up fabric | 28.8 TB/s per switch tray; full all-to-all |
| Spectrum-6 | scale-out Ethernet | 102.4 Tb/s per chip, 512 × 200 Gb/s |

Together: **NVL144** (one rack, 144 Rubin GPUs) gives ~7.2 PFLOPS NVFP4 training per rack (×3.5 vs Blackwell NVL72), with 41 TB of HBM4 in a single coherent fabric.

For frontier dense at this repo's 427B scale: **a single Rubin GPU now fits the entire model in one HBM4 stack** (288 GB > 214 GB int4 / 214 GB fp4 weights + 220 GB fp4 KV at 1M ≈ 434 GB, just over one Rubin → comfortably 2× Rubin instead of 4× B200 or 16× H100).

### NVIDIA Blackwell Ultra — B300 / GB300 NVL72 (late 2025+)

The mid-cycle refresh between B200 and Rubin. Per-GPU specs (verbatim from NVIDIA datasheet):

| Property | B300 in **GB300 NVL72** | B300 in **HGX B300** |
|---|---|---|
| GPU memory | 279 GB HBM3E | 270 GB HBM3E |
| Memory bandwidth | 8 TB/s | 7.7 TB/s |
| TDP | up to 1400 W | up to 1100 W |
| FP4 tensor core (sparse \| dense) | 20 \| **15 PFLOPS** | 18 \| **14 PFLOPS** |
| FP8 / FP6 tensor core (sparse) | 10 PFLOPS | 9 PFLOPS |
| FP16 / BF16 (sparse) | 5 PFLOPS | 4.5 PFLOPS |
| TF32 (sparse) | 2.5 PFLOPS | 2.2 PFLOPS |
| INT8 (sparse) | 330 TOPS | 307 TOPS |
| FP32 (non-tensor) | 80 TFLOPS | 75 TFLOPS |
| FP64 tensor core | 1.3 TFLOPS | 1.2 TFLOPS |
| NVLink (5th gen) | 1.8 TB/s | 1.8 TB/s |
| PCIe | Gen6, 256 GB/s | Gen6, 256 GB/s |
| MIG partitions | 7 | 7 |

**Per-rack (GB300 NVL72 — 72 Blackwell Ultra GPUs + 36 Grace CPUs / 2592 Arm Neoverse V2 cores):**

| Property | Value |
|---|---|
| Total FP4 tensor core (sparse \| dense) | 1440 \| 1080 PFLOPS |
| Total FP8/FP6 tensor core (sparse) | 720 PFLOPS |
| Total fast memory | 37 TB |
| Total memory bandwidth | 576 TB/s |
| Total NVLink switch bandwidth | 130 TB/s |

Per-format support (same as B200, with dense FP4 throughput numbers — divide sparse by 2):

| Format | Native? | Throughput per GPU (dense) |
|---|:---:|---|
| fp32 | yes (acc) | 80 TFLOPS |
| tf32 | yes | 1.25 PFLOPS dense (2.5 sparse / 2) |
| bf16 / fp16 | yes | 2.5 PFLOPS dense |
| fp8 (E4M3 / E5M2) | yes | 5 PFLOPS dense |
| FP6 | yes | 5 PFLOPS dense |
| MXFP8 | **yes (native)** | 5 PFLOPS dense |
| MXFP6 | **yes** | 7.5 PFLOPS dense |
| MXFP4 | **yes (native)** | **15 PFLOPS dense** (GB300) / 14 PFLOPS (HGX B300) |
| fp4 (raw E2M1) | yes | 15 PFLOPS dense |
| NVFP4 | software via MXFP4 | slower than Rubin native |
| int8 | yes | 165 TOPS dense |
| int4 | yes | 330 TOPS dense |

For the 427B-class model in this repo: **a single B300 GPU with 279 GB HBM3E fits more than the entire fp8 weight footprint (~430 GB)** for a tightly-quantized variant, and 2× B300 comfortably fits weights + 1M-context KV at fp8. This is one full GPU-tier reduction vs B200.

### NVIDIA Blackwell (B200 / GB200 NVL72, 2025)

The first-generation Blackwell. Per-GPU specs:

| Property | B200 |
|---|---|
| GPU memory | 192 GB HBM3e |
| Memory bandwidth | ~8 TB/s |
| FP4 tensor core (dense) | ~10 PFLOPS |
| FP8/FP6 tensor core (sparse) | ~5 PFLOPS dense (~10 sparse) |
| BF16 tensor core (dense) | ~2.5 PFLOPS |
...
| MXFP6 | **yes** | ~3× |
| MXFP4 | **yes (native)** | ~4× |
| NVFP4 | software-emulated via MXFP4 path | slower than Rubin native |
| fp4 (raw E2M1) | **yes** | ~4× |
| int8 | yes | ~2× |
| int4 | yes | ~4× |

Memory: 192 GB HBM3e per GPU (vs H100's 80 GB). FP8 mixed-precision is the **default training path** on Blackwell. Operators with a Rubin upgrade path on the horizon should target NVFP4-compatible serving workflows now (i.e. quantize via MXFP4 today; the same artifacts run at 5× throughput on Rubin tomorrow).

### About sparse vs dense throughput numbers

NVIDIA datasheets report tensor-core throughput in two columns: **sparse** (assumes the model uses 2:4 structured sparsity — half the elements in every 4-element block are zero, hardware skips them) and **dense** (no sparsity assumed, half the sparse number).

For a frontier-dense model trained without sparsity-aware pruning, the **dense** numbers are what you measure in practice. Examples:

- B300 in GB300 NVL72: **15 PFLOPS FP4 dense** (advertised 20 PFLOPS sparse).
- Rubin: **35 PFLOPS NVFP4 training, 50 PFLOPS NVFP4 inference** (these are the headline numbers; the dataset/blog doesn't break sparse-vs-dense for Rubin yet).
- H100: **2 PFLOPS FP8 dense** (advertised 4 PFLOPS sparse).

If your training run uses 2:4 sparsity (rare for dense pretraining; common in some inference quantization recipes), use the sparse number. Otherwise halve it.
...
flowchart TD
    ...
    C -->|Rubin R100 / NVL144| RT[NVFP4 mixed via Transformer Engine 2<br/>master weights fp32<br/>MXFP8 / fp8 also OK]
    ...
    J -->|Rubin| RJ[NVFP4 KV cache<br/>+ NVFP4 weight-only<br/>Single GPU fits 427B at 10M ctx]
    ...
    K -->|Rubin| RK[NVFP4 weights + NVFP4 KV<br/>5x B200 throughput at half the GPUs]
...
|---|---|---|
| Pretrain a frontier-dense model | **Rubin R100** | **`mixed_precision: nvfp4`** (NVFP4 mixed via TE2), MXFP8 also OK |
...
| Serve at low latency | **Rubin** | **NVFP4 weights + NVFP4 KV** (50 PFLOPS, 1 GPU fits 427B int4) |
| Serve at low latency | Blackwell | fp8 weights + fp8 KV cache |
| Serve at minimum cost | Rubin | NVFP4 weights + NVFP4 KV cache |
